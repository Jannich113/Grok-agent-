...
> **Option C (sandbox) of [Jannich113/Grok-agent-](https://github.com/Jannich113/Grok-agent-).**
> Use this file only inside **Grok App Builder**: isolated Linux `/workspace`,
> user has chat + live preview only, app binds `0.0.0.0:8080`, you own
> `/workspace/startup.sh`. Outside that environment, use the portable root
> `AGENTS.md` (Option A/B). Depth still lives in `.grok/skills/` and in
> `sandbox/references/` (copied to `.grok/references/` on install).
